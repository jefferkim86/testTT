...
{  
	...
	{
		return spClass('db_member')->find(array('domain'=>$domain),'','uid,username,domain,blogtag,sign,num,flow,flowme,likenum,local,logtime');
	}
	
	function getByUid($uid)
	{
		return spClass('db_member')->find(array('uid'=>$uid),'','uid,username,domain,blogtag,sign,num,flow,flowme,likenum,local,logtime');
	}
	
	function getByBid($bid)
	{
		$rs = spClass('db_blog')->find(array('bid'=>$bid),'','uid');
		return spClass('db_member')->find(array('uid'=>$rs['uid']),'','uid,username,domain,blogtag,sign,num,flow,flowme,likenum,local,logtime');
	}
